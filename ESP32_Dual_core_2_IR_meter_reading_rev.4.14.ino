...
void updateThingsboard()
{
  ...
  if ( ! ServerConnected) {
    Serial.println (F( "Thingsboard server is not connected"));
  }
  else {
    Serial.println (F( "Thingsboard server connected ******* OKAY "));
  };
  connection_tries = 0;
...
void delay_count( unsigned long delayed)
{
  ...
  if ( analogRead(power_loss_pin_detection) < 100) {
